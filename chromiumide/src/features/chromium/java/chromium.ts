...
interface BuildConfigDepsInfo {
  ...
  // gen/third_party/android_sdk/android_sdk_java.params.json.
  is_prebuilt?: boolean;
...
async function isUsefulSourceJar(
  ...
  // explicitly exclude them here, but actually they don't appear in *.params.json at all.)
...
async function processConfigJson(
  ...
  if (config.target_sources_file) {
    ...
    (config.is_prebuilt || config.unprocessed_jar_path.startsWith('../'))
  ) {
    // This is a prebuilt jar file. Add it to the class path.
    classJarSet.add(path.resolve(outDir, config.unprocessed_jar_path));
  }

  if (config.bundled_srcjars) {
    for (const sourceJarRelPath of config.bundled_srcjars) {
...
/**
 * Processes *.params.json files in the output directory.
 */
